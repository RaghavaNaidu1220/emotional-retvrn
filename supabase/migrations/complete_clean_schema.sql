-- Complete Clean Schema for Emotional Spiral App
-- This will create a unified, consistent database structure
...
CREATE TABLE public.sessions (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    session_type TEXT NOT NULL, -- 'voice_journal', 'ai_journal', 'conversation', etc.
    transcript TEXT,
    audio_url TEXT,
    emotional_tone JSONB DEFAULT '{}',
    growth_themes TEXT[],
...
CREATE TABLE public.user_progress (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    metric_type TEXT NOT NULL, -- 'spiral_stage', 'emotional_awareness', etc.
...
CREATE INDEX idx_emotion_analyses_journal_id ON public.emotion_analyses(journal_id);
CREATE INDEX idx_voice_journal_entries_user_id ON public.voice_journal_entries(user_id);
CREATE INDEX idx_voice_journal_entries_created_at ON public.voice_journal_entries(created_at DESC);
CREATE INDEX idx_ai_journal_entries_user_id ON public.ai_journal_entries(user_id);
CREATE INDEX idx_ai_journal_entries_created_at ON public.ai_journal_entries(created_at DESC);
...
-- This will be automatically cleaned up by RLS policies
